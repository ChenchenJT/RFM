...
def greedy(model, data, vocab2id, max_len=20, encode_outputs=None, init_decoder_states=None, init_feedback_states=None):
    BOS = vocab2id[BOS_WORD]
    EOS = vocab2id[EOS_WORD]
    UNK = vocab2id[UNK_WORD]
    PAD = vocab2id[PAD_WORD]
    ...
    if init_feedback_states is None:
        feedback_outputs = model.init_feedback_states(data, encode_outputs, decoder_states)

    decoder_input = new_tensor([BOS] * batch_size, requires_grad=False)
    all_inputs = decoder_input.unsqueeze(1)
    ...
    for t in range(max_len):
        ...
        all_inputs = torch.cat([all_inputs, decoder_input.unsqueeze(1)], dim=-1)  # 所有生成过的词
    # 查看内容
    # print('test content: \n', all_inputs)
...
def beam(model, data, vocab2id, max_len=20, width=5, encode_outputs=None, init_decoder_states=None):
    ...
    for i in range(batch_size):
        next_fringe += [Node(parent=None, state=get_data(i, decode_outputs), word=BOS_WORD, value=BOS, cost=0.0,
                             encode_outputs=get_data(i, encode_outputs), data=get_data(i, data), batch_id=i)]
        results[i] = []
