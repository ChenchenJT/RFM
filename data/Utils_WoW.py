...
def load_answer(file):
    ...
    with codecs.open(file, encoding='utf-8') as f:
        for line in f:
            ...
            assert len(
                temp) == 4, "all_previous_query_id;all_previous_query_id;all_previous_query_id	current_query_id	background_id;background_id 	response_content"
            if len(temp[0]) < 1:
                temp[0] = []
            else:
                temp[0] = temp[0].split(';')
            temp[2] = temp[2].split(';')
            temp[3] = nltk.word_tokenize(temp[3])
            answer.append(temp)
    return answer
    ...
    with codecs.open(file, encoding='utf-8') as f:
        for line in f:
            ...
            if temp[0] in poolset:
                passage[temp[0]] = ' [SEP] '.join(
                    [' '.join(nltk.word_tokenize(sent)) for sent in nltk.sent_tokenize(temp[1])]).split(' ')  # list的形式
...
def load_query(file):  # query_id	query_content
    ...
    with codecs.open(file, encoding='utf-8') as f:
        for line in f:
            ...
            query[temp[0]] = nltk.word_tokenize(temp[1])  # {1_1:[query_tokens],}
    return query
...
def load_default(dataset, answer_file, passage_file, pool_file, qrel_file, query_file, topk=None, randoms=1):
    random.seed(1)
    ...
    for i in range(len(answer)):
        for j in range(randoms):
            # c_id is a lis，q_id is string，p_id is a list，ans is a list
            c_id, q_id, knowledge_shifting_id, response = answer[i]
            ...
            if len(c_id) == 0:
                ...
                if j == 0:
                    ...
                    knowledge_tracking_pool[0], knowledge_tracking_pool[j] = knowledge_tracking_pool[j], \
                                                                             knowledge_tracking_pool[0]
            ...
            if i == 0:
                ...
                knowledge_shifting_pool[0], knowledge_shifting_pool[i] = knowledge_shifting_pool[i], \
                                                                         knowledge_shifting_pool[0]
